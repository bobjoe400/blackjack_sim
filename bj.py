...
global_strategy_list: list[list[str]] = []

global_dealer_info = [0, [['']], 0, 0, [False], [False], [False]]
...
def play_hand_dealer(deck: list[str], 
    ...
    while((hand_info := get_hand_info(drawn_cards))[const.HAND_SUM_INDEX] < const.BUST_NUMBER and (hand_info[const.HAND_SUM_INDEX] <= 17)):
        ...
        hit(drawn_cards, deck, global_dealer_info)
    ...
    if(hand_info[const.HAND_SUM_INDEX] > const.BUST_NUMBER):
        ...
        stand(drawn_cards, deck, global_dealer_info)
...
def play_hand_player(deck: list[str], 
    ...
    while(curr_hand_index < num_hands):
        ...
        while continue_current_hand:
            ...
            strategy = get_suggest_play(curr_hand, player_info[const.PLAYER_NUM_SPLITS_INDEX], dealer_card, global_strategy_list)
    ...
    print_message("Fishished with ", player_num, "'s hand.\n", sep='')

    return num_cards_drawn

def get_payout_ratio(player_info: PLAYER_INFO_TYPE, 
                     player_hand_info_list: list[list[HAND_INFO_TYPE]], 
                     hand_num: int, 
                     dealer_hand_info: HAND_INFO_TYPE, 
                     dealer_blackjack: bool) -> float:
    curr_payout_ratio = const.LOSE_PAYOUT_RATIO
...
def calc_winners(current_felt, active_players_list, hand_ratio_results):
    ...
    for player_num in range(const.NUM_PLAYERS):
        ...
        for hand_num in range(len(curr_hand_info_list)):
            ...
            hand_ratio_results[player_num] += [payout_ratio]
...
def print_hand_results(win_and_lost_list: list[list[int]], active_players_list, hand_ratio_results):
    print_message("\nHand over! Here are the results:")
    
    for player_num in range(const.NUM_PLAYERS):
        if active_players_list[player_num]:
            curr_ratios = hand_ratio_results[player_num]
            for hand_num in range(len(curr_ratios)):
                curr_payout_ratio = curr_ratios[hand_num]
                payout_desc = const.PAYOUT_RATIO_DESCRIPTIONS[curr_payout_ratio]
                print_message("\tOn hand", hand_num, "player", player_num, payout_desc)
...
def run_game(deck, stop_point):
    ...
    while True:
        while(drawn_cards < stop_point):
            print_message("\n-------------------------")
            print_message("Starting new hand.")
            print_message("-------------------------\n")
            ...
            hand_ratio_results: list[list[float]] = [[] for i in range(const.NUM_PLAYERS)]

            win_loss_list = calc_winners(current_felt, active_players_list, hand_ratio_results)
            
            total_chip_change_list = print_hand_results(list(win_loss_list), active_players_list, hand_ratio_results)
...
if __name__ == "__main__":
    global_strategy_list = load_strategy(const.STRATEGY_FILENAME)
    (deck, stop_point) = generate_deck(const.NUM_DECKS)
